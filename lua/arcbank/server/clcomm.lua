...
net.Receive( "arcbank_comm_lang", function(length,ply)
	...
	if string.StartWith(steamid,"RECIEVED DUH FOOKING CHUNK ||||") then
		local progr = string.Explode("/",string.Replace(steamid,"RECIEVED DUH FOOKING CHUNK ||||",""))
		if tonumber(progr[2]) == #ARCBank.JSON_Lang then
			if tonumber(progr[1]) == ply._ARCBank_Lang_Place then
				...
				net.WriteString(tostring(ARCBank.JSON_Lang[ply._ARCBank_Lang_Place]))
				net.Send(ply)
			else
				net.Start("arcbank_comm_lang")
				net.WriteInt(ARCBANK_ERROR_CHUNK_MISMATCH,ARCBANK_ERRORBITRATE)
		...
		MsgN("ARCBank: Failure Sending language to "..tostring(ply))
	end
end)
---------------------
-- ADMIN FUNCTIONS --
